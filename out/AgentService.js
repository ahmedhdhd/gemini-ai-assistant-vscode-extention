...
- Don't be afraid to be unconventional when appropriate`,
    ...
    loadCustomAgents() {
        const customAgents = this.context.globalState.get('customAgents', []);
        customAgents.forEach(agent => {
            this.agents.set(agent.id, agent);
        });
    }
    saveCustomAgents() {
        const customAgents = Array.from(this.agents.values()).filter(agent => !this.isDefaultAgent(agent.id));
        this.context.globalState.update('customAgents', customAgents);
    }
    loadCurrentAgent() {
        this.currentAgentId = this.context.globalState.get('currentAgentId', 'default');
    ...
    getSystemPromptForCurrentAgent() {
        return this.getCurrentAgent().systemPrompt;
    }
    getTemperatureForCurrentAgent() {
